...
class Tx_YagThemepackJquery_ViewHelpers_CrossSlideViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractTagBasedViewHelper
{
    ...
    protected function buildPanAndZoom($imageSettings)
    {
        ...
        if ($imageSettings['pan']['alternate']) {
            $this->panReversed = $this->panReversed ? false : true;
        }
        if ($imageSettings['zoom']['alternate']) {
            $this->zoomReversed = $this->zoomReversed ? false : true;
        }

        if ($this->panReversed) {
            $pan = array_reverse($pan);
        }
        if ($this->zoomReversed) {
            $zoom = array_reverse($zoom);
        }
